...
# --- FUNÇÃO CARTOGRÁFICA TÉCNICA (CORES E TRAMAS) ---
def gerar_mapa_tecnico(user_gdf):
    ...
    # 2. Mapa Base (Redundância Google/Esri)
    mapa_sucesso = False
    ...
    # 3. Cruzamento Geospacial para Desenho das Tramas
    # Nota: Assume-se que os ficheiros existem na pasta data/
    for nome, estilo in estilos.items():
        ...
        if os.path.exists(path):
            ...
            # Interseção entre a área fiscalizada e a servidão
    ...
    legend_elements.append(Line2D([0], [0], color='red', linewidth=3, label='Area Alvo da Fiscalizacao'))
...
def exportar_pdf(texto_ia, mapa_path):
    ...
    if os.path.exists(mapa_path):
        ...
        pdf.set_y(180) # Posicionamento seguro para a legenda e texto
    
    pdf.set_font("Arial", '', 10)
    txt_fpdf = texto_ia.encode('latin-1', 'ignore').decode('latin-1')
    pdf.multi_cell(0, 6, txt_fpdf)
    
    pdf_name = "Relatorio_Final_Tecnico.pdf"
    pdf.output(pdf_name)
    return pdf_name

# --- INTERFACE ---
st.title("🛡️ Fiscalização SIG: Análise Técnica e Jurídica")
...
if file:
    ...
    with col_res:
        st.subheader("📋 Resumo Geospacial")
        st.write(f"**Area Total:** {area:.2f} m2 [cite: 31, 38]")
        
        if api_key:
            if st.button("🤖 Gerar Parecer e Cartografia Técnica"):
                with st.spinner('A processar cruzamento de camadas e redação IA...'):
                    dados_ia = {
                        "area": f"{area:.2f} m2",
                        "divergencia": "Aterro detetado em zona de culturas temporarias [cite: 33, 40]",
                    ...
                    st.success("Relatório Técnico Concluído!")
                    with open(pdf_final, "rb") as f:
                        st.download_button("📥 Descarregar PDF Técnico", f, file_name=pdf_final)
